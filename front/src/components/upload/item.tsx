...
export default function Item(itemData: UploadItem, handleRemove: any) {
    ...
    return (
        <div className="flex">
            ...
            {
                isModified ? <input ref={inputRef} name={"filePath"} type={"file"} onChange={handleInputChange}/> :
                    <div>{item.filePath as string}</div>
            }
            {isModified ? <button onClick={() => setIsModified(false)}>수정완료</button> :
                <button onClick={() => setIsModified(true)}>수정하기</button>}
